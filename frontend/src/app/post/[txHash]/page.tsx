  ...
  const versionEntries: VersionEntry[] = [
    ...versions.map(v => ({
      ...
      content: {
        ...
        updated_at:   v.metadata.updated_at ?? v.metadata.created_at,
        is_paid:      (v.metadata as Record<string, unknown>).is_paid as boolean ?? false,
        unlock_price: (v.metadata as Record<string, unknown>).unlock_price as number ?? 0,
        content:      v.content,
    ...
    })),
    {
      ...
      content: {
        ...
        updated_at:   (metadata as Record<string, unknown>).updated_at as number ?? metadata.created_at,
        is_paid:      (metadata as Record<string, unknown>).is_paid as boolean ?? false,
        unlock_price: (metadata as Record<string, unknown>).unlock_price as number ?? 0,
        content,
      } as VersionEntry['content'],
